"""
parsers/hibata.py
Source  : https://hibata.id/daerah/pohuwato/
...
def get_article_links(listing_url: str) -> tuple[list[dict], str | None]:
    ...
    # Each article card has: article.hentry > div.box-item/box-bigitem > ... h2.entry-title > a
    for article in soup.select("article.hentry"):
        ...
        # Date is in time.entry-date[datetime] — ISO 8601 with TZ offset
    ...
    if not results:
        raw = extract_article_links(
            soup, listing_url, "h2.entry-title a", domain_hint="hibata.id"
        )
        for l in raw:
...
def parse_article(url: str) -> dict | None:
    ...
    # Date — prefer time.entry-date.published[datetime] (ISO 8601)
    date_raw = ""
