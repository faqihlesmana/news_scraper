"""
parsers/gorontalopost.py
Source: https://gorontalopost.co.id/category/pohuwato-madani/
Listing:  h2.jeg_post_title a, h3.jeg_post_title a
Article:  h1.jeg_post_title, [class*='entry-content'], [class*='date'], [class*='author']
"""
import json, re
from parsers.base_parser import fetch, fetch_full_article_content, normalize_date, find_next_page, extract_article_links

SOURCE_SLUG = "gorontalopost"
SOURCE_NAME = "Gorontalo Post"
...
def get_article_links(listing_url: str) -> tuple[list[str], str | None]:
    ...
    links = extract_article_links(soup, listing_url,
                                   ...
                                   domain_hint="gorontalopost.co.id")
...
def parse_article(url: str) -> dict | None:
    ...
    image = ""
    img = soup.select_one("[class*='featured'] img, article img")
    if img:
        image = img.get("src", "")
