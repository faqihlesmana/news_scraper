"""
parsers/seputarpohuwato.py
Source: https://seputarpohuwato.com/
Listing:  h3 a  (no class on h3)
...
SOURCE_SLUG = "seputarpohuwato"
SOURCE_NAME = "Seputar Pohuwato"
...
def get_article_links(listing_url: str) -> tuple[list[str], str | None]:
    ...
    links = extract_article_links(soup, listing_url,
                                   "h3 a, h2.entry-title a",
                                   domain_hint="seputarpohuwato.com")
    # filter: must have slug-like path
    links = [l for l in links if "-" in l.rstrip("/").split("/")[-1]]
...
def parse_article(url: str) -> dict | None:
    ...
    author = ""
    for sel in ["[class*='author']", ".author"]:
